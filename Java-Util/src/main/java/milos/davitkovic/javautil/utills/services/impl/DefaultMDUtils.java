package milos.davitkovic.javautil.utills.services.impl;

import lombok.extern.slf4j.Slf4j;
import milos.davitkovic.javautil.utills.annotations.UtilClass;
import milos.davitkovic.javautil.utills.services.MDUtils;
import milos.davitkovic.javautil.utills.services.impl.utils.Array.ArrayUtils;
import milos.davitkovic.javautil.utills.services.impl.utils.Array.list.ListUtils;
import milos.davitkovic.javautil.utills.services.impl.utils.Array.set.SetUtils;
...
import milos.davitkovic.javautil.utills.services.impl.utils.Number.Integer.IntegerUtils;
import milos.davitkovic.javautil.utills.services.impl.utils.Number.NumberUtils;
import milos.davitkovic.javautil.utills.services.impl.utils.Time.TimeUtils;
...
public class DefaultMDUtils implements MDUtils {
    ...
    @Autowired
    private FindIOUtils fileIOUtils;
    @Autowired
    ...
    public String listFiles(final String folderName) {
        ...
        return findIOUtils.listFilesInSpecificFolder(folderName);
    }
    ...
    public <T> void writeInResourceFile(final String fileName, final String folderName, final Collection<T> input) throws IOException {
        ...
        if (CollectionUtils.isEmpty(inputText)) {
            log.warn("WARN-WRITE-IN-RESOURCE-FILE, List with lines for write is empty. Nothing to write down in a file {} from a folder {}.", fileName, folderName);
            return;
        }

        try {
            writeIOUtils.writeInResourceFile(fileName, folderName, inputText);
            log.info("WRITE-IN-RESOURCE-FILE, {} lines of text have been written down in file {} placed in folder {}", inputText.size(), fileName, folderName);
        } catch (IOException e) {
